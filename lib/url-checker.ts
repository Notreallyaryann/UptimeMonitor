...
export async function checkAllDueUrls() {
...
// Export this for immediate checks when URL is added
export async function checkSingleUrl(urlEntry: any) {
    ...
    return { success: isSuccess, statusCode, errorMessage }
}
...
async function handleAlert(urlEntry: any, errorMessage: string | null, statusCode: number | null) {
    ...
    if (!activeAlert || !activeAlert.notificationSent) {
        ...
        try {
            ...
            console.log(`📧 Alert sent for ${urlEntry.url}`)
        } catch (emailError) {
